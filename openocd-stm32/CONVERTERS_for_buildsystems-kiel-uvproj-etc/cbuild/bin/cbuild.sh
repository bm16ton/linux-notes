...
# header
header() {
  echo "($(basename "$0")): Build Invocation $version (C) 2020 ARM" >&${out}
}
...
usage() {
  ...
  echo "  [--toolchain=<Toolchain> --outdir=<OutDir> --intdir=<IntDir> <MakefileTarget>]"
  echo ""
  ...
  echo "  <MakefileTarget>        : optional makefile target name, for example 'clean'"
  echo "  --quiet                 : suppress output messages except build invocations"
  echo "  --merge                 : update of config with ccmerge"
...
if [ -z "${intdir}" ]
  ...
  if [ -z "${cprj_intdir}" ]
    then
    cprj_objdir=$(echo "$(grep -o 'obj=.*' "${projectdir}${project}.cprj" | cut -f2 -d\")") # Deprecated
    if [ -z "${cprj_objdir}" ]
      then
      abs_intdir="${projectdir}IntDir/"
    else
      abs_intdir="${projectdir}${cprj_objdir}"
    fi
  else
...
if [ $merge -eq 0 ]
  then
  # call cbuildgen to generate makefile
  cbuildgen make "${projectdir}${project}.cprj" --toolchain="${toolchain}" --update="${update}" --outdir="${outdir}" --intdir="${intdir}" ${quiet_opt}
  else
  # call cbuildgen with merge to generate makefiles
  cbuildgen make --merge=ccmerge "${projectdir}${project}.cprj" --toolchain="${toolchain}" --update="${update}" --outdir="${outdir}" --intdir="${intdir}" ${quiet_opt}
fi
...
# 'make' needs to run in projectdir
pushd "${projectdir}"

if [ $merge -eq 1 ]
  then
  # merge config files (ccmerge)
  make -s -j${jobs} -r -f "${abs_intdir}${project}_cfg.mak" ${m_target}
  if [ $? -ne 0 ]
    then
    echo "make ${abs_intdir}${project}_cfg.mak failed!" >&${err}
    popd
    exit 1
  fi
fi
...
# build
make -j${jobs} -r -f "${abs_intdir}${project}.mak" ${m_target}
if [ $? -ne 0 ]
  then
  echo "make ${abs_intdir}${project}.mak failed!" >&${err}
  popd
  exit 1
fi
