...
#define PWREXT_ENABLE()					do {\
											core_interfaces.gpio.out(PWREXT_EN_PORT, 1 << PWREXT_EN_PIN, 1 << PWREXT_EN_PIN);\
...
/****************************** Pod selector ***********************/
#if defined(HW_HAS_SELECT) && (HW_HAS_SELECT == 1)
#define SELECT_PORT					1
#define SELECT_PIN					12

#define SELECT_INIT()					do {\
											core_interfaces.gpio.init(SELECT_PORT);\
											SELECT_PL();\
										} while (0)
#define SELECT_PR()						do {\
											core_interfaces.gpio.out(SELECT_PORT, 1 << SELECT_PIN, 1 << SELECT_PIN);\
											core_interfaces.gpio.config_pin(SELECT_PORT, SELECT_PIN, GPIO_OUTPP);\
										} while (0)
#define SELECT_PL()						do {\
											core_interfaces.gpio.out(SELECT_PORT, 1 << SELECT_PIN, 0 << SELECT_PIN);\
											core_interfaces.gpio.config_pin(SELECT_PORT, SELECT_PIN, GPIO_OUTPP);\
										} while (0)
#endif
