...
int rateTable[7] = {100,250,400,500,1000,2000,4000};
//int rateTable[1] = {4000};
int main(void) {
...
//Process the data request, save the received data at g_rxBuff.
            if (g_rxBuff[0] == '0'); //(Unimplemented in this version)
            if (g_rxBuff[0] == '1') {
//                HSADC_init((int)rateTable[g_rxBuff[1]-'0']);
                HSADC_init();
                samplingADC();
                usbDumpConnected = true;
                Board_LED_Set(0,true);
                //GPDMA_capture(&g_txBuff[0], 128);
				ADC_DMA_captureUSB(&g_txBuff[0], &g_txBuff1[0],&g_txBuff2[0], &g_txBuff3[0], 2048);
			}
			if (g_rxBuff[0] == '2') {
